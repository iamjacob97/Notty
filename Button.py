...
class Label:
    def __init__(self, pos, text_input, font, base_colour, rotate = 0):
        ...
        self.image = pygame.transform.rotate(self.font.render(self.text_input, True, self.base_colour), rotate)
...
class Button(Label):
    ...
    def __init__(self, image, pos, text_input, font, base_colour, hovering_colour):
        super().__init__(pos, text_input, font, base_colour)
        # Assign the image to the button
    ...
    def changeColour(self, position):
        ...
        if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top, self.rect.bottom):
            self.image = self.font.render(self.text_input, True, self.hovering_colour)
        # Otherwise, change text colour back to base colour
        else:
            self.image = self.font.render(self.text_input, True, self.base_colour)
